...
console.log(`miasta w Polsce: ${cities.length}`)
console.log(`Ludzie w sumie: ${cities.map(it=>it.people).reduce((pr, curr)=>pr + curr)}`)
console.log(`Pierwsze ponad 10000: ${cities.find((city=>city.people>10000)).name}`)
console.log("Ponad średnią:")
avg = cities.map(it=>it.people).reduce((pr, curr)=>pr + curr)/cities.length
console.log(cities.filter(city=>city.people > avg).map(city=>city.name))

wiecejDuzych = cities.filter(city=>city.people>10000).length > cities.length/2

console.log("Miast z ponad 10000 mieszkanców jest " +(wiecejDuzych ? "wiecej" : "nie wiecej" )+ " niż mniejszych")
